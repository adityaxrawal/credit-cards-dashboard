    ...
    <div className="space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-card-bg rounded-xl p-4 border border-white/5 card-shadow">
            <p className="text-sm text-secondary-text mb-1">Total Outstanding</p>
            <p className="text-2xl font-bold font-mono text-semantic-red">
              {formatCurrency(summary.totalOutstanding)}
            </p>
          </div>
          <div className="bg-card-bg rounded-xl p-4 border border-white/5 card-shadow">
            <p className="text-sm text-secondary-text mb-1">Monthly EMI</p>
            <p className="text-2xl font-bold font-mono text-accent-orange">
              {formatCurrency(summary.totalMonthlyEmi)}
            </p>
          </div>
          <div className="bg-card-bg rounded-xl p-4 border border-white/5 card-shadow">
            <p className="text-sm text-secondary-text mb-1">Active Loans</p>
            <p className="text-2xl font-bold font-mono text-primary-text">
              {summary.activeLoans}
            </p>
          </div>
          <div className="bg-card-bg rounded-xl p-4 border border-white/5 card-shadow">
            <p className="text-sm text-secondary-text mb-1">Total Loans</p>
            <p className="text-2xl font-bold font-mono text-primary-text">
              {summary.totalLoans}
            </p>
          </div>
      ...
      {summary?.upcomingEmis && summary.upcomingEmis.length > 0 && (
        <div className="bg-card-bg rounded-xl p-5 border border-white/5 card-shadow">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin">
            {summary.upcomingEmis.map((emi) => (
              <div key={emi.loanId} className="flex-shrink-0 bg-hover-bg rounded-lg p-3 min-w-[180px]">
                ...
                <p className="text-lg font-bold font-mono text-accent-orange mt-1">{formatCurrency(emi.emiAmount)}</p>
      ...
      <div className="space-y-4">
        {Array.isArray(loans) && loans.map((loan) => {
          ...
          return (
            <div
              key={loan.id}
              className="bg-card-bg rounded-xl p-5 border border-white/5 hover:border-primary-green/30 transition-colors cursor-pointer group card-shadow"
            >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-lg font-bold font-mono text-primary-text">{formatCurrency(loan.currentOutstanding)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-text">EMI Amount</p>
                      <p className="text-lg font-semibold font-mono text-accent-orange">{formatCurrency(loan.emiAmount || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-text">Interest Rate</p>
                      <p className="text-lg font-semibold font-mono text-primary-text">{loan.interestRate}%</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-text">Remaining EMIs</p>
                      <p className="text-lg font-semibold font-mono text-primary-text">{loan.remainingEmis || '-'}</p>
                    </div>
                  </div>
