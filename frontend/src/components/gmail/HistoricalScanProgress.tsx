...
import { Play, Pause, CheckCircle, AlertCircle, Clock, Loader } from 'lucide-react';
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-lg bg-gray-50 p-3 border border-gray-200">
          ...
          <p className="text-2xl font-bold text-gray-900">{progress.totalMessages.toLocaleString()}</p>
        </div>
        <div className="rounded-lg bg-gray-50 p-3 border border-gray-200">
          <p className="text-xs text-gray-600 mb-1">Processed</p>
          <p className="text-2xl font-bold text-blue-600">{progress.processedMessages.toLocaleString()}</p>
        </div>
        <div className="rounded-lg bg-gray-50 p-3 border border-gray-200">
          <p className="text-xs text-gray-600 mb-1">Extracted</p>
          <p className="text-2xl font-bold text-green-600">{progress.extractedTransactions.toLocaleString()}</p>
        </div>
        <div className="rounded-lg bg-gray-50 p-3 border border-gray-200">
          <p className="text-xs text-gray-600 mb-1">Failed</p>
          <p className="text-2xl font-bold text-red-600">{progress.failedMessages.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-gray-200 space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium">{progress.duplicateMessages.toLocaleString()}</span>
        </div>
        {progress.startedAt && (
          <div className="flex justify-between text-sm">
