...
export class TransactionPrefilter {
    ...
    // Default Threshold: Lower for higher recall (catch more transactions, let ML filter)
    private static DEFAULT_THRESHOLD = 12;
    ...
    static evaluate(text: string, thresholdOverride?: number): EvaluationResult {
        ...
        // Debug mode: log prefilter decisions
        if (process.env.PREFILTER_DEBUG === 'true') {
            console.log(`[Prefilter] Score: ${score}/${threshold}, Signals: [${triggeredSignals.join(', ')}], Process: ${result.shouldProcess}`);
        }
