...
export async function runReconciliation(req: Request, res: Response) {
    try {
        ...
        const result = {
            ...
            missingIds: missingInDb.slice(0, 50),
            extraIds: []
        };
 ...
 */
export async function getOllamaMetrics(req: Request, res: Response) {
    try {
        const { OllamaService } = await import('../services/ollama.service');
        const { TransactionPrefilter } = await import('../utils/TransactionPrefilter');

        const metrics = OllamaService.getMetrics();
        const signals = TransactionPrefilter.getSignals();
        ...
        res.json({
            ...
            prefilter: {
                signals: signals,
                threshold: parseInt(process.env.PREFILTER_THRESHOLD || '12', 10)
            }
        });
    } catch (error) {
        console.error('[SyncController] Metrics error:', error);
        res.status(500).json({
...
/**
 * Stream real-time notifications (SSE)
 * GET /api/sync/stream
 */
export async function streamNotifications(req: Request, res: Response) {
    NotificationService.subscribe(res);
}
