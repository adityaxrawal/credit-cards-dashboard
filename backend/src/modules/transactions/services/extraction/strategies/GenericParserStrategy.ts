...
export class GenericParserStrategy implements StatementParserStrategy {
    public name = 'GENERIC';

    public canHandle(bankName: string): boolean {
        // Fallback for any bank not handled by specific strategies
        return true;
    }
    ...
    public parse(text: string, defaultDate: string): ExtractedStatementTransaction[] {
        ...
        for (const line of lines) {
            ...
            transactions.push({
                ...
                bank: 'UNKNOWN', // Wrapper will set this
                currency: 'INR',
                category: this.categorizeTransaction(cleanMerchantName),
                confidence: 0.8, // Lower confidence for generic
                evidence: {
                    rawLine: cleanLine
                }
            });
