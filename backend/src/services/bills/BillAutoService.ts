...
import { CleanEmailContent } from '../gmail/sanitize/sanitizer';
    ...
    static async createBillFromStatement(
        ...
        try {
            ...
            if (statement.cardLast4) {
                const cardResult = await pool.query(
                    `SELECT id FROM instruments 
                     ...
                     LIMIT 1`,
                    [userId, statement.cardLast4]
                );

                if (cardResult.rows.length > 0) {
                    instrumentId = cardResult.rows[0].id;
                }
            }
            ...
            const existingBill = await pool.query(
                ...
                [userId, instrumentId, statement.dueDate.getMonth() + 1, statement.dueDate.getFullYear()]
            );
            ...
            const result = await pool.query(
                ...
                RETURNING id, user_id, instrument_id, amount, due_date, status`,
                [
                    userId,
                    instrumentId,
                    `${statement.bankName} Credit Card Statement`,
                    statement.billAmount,
                    statement.minDueAmount,
                    statement.dueDate,
                    'Credit Card Payment',
                ]
            );
            ...
            return {
                id: bill.id,
                userId: bill.user_id,
                instrumentId: bill.instrument_id,
                amount: bill.amount,
