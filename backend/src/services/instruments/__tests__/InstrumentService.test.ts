...
describe('InstrumentService', () => {
    ...
    describe('upsertInstrument', () => {
        test('should insert instrument and invalidate cache', async () => {
            const mockClient = {
                query: jest.fn().mockResolvedValue({ rows: [mockInstrument] }),
                release: jest.fn(),
            };
            (pool.connect as jest.Mock).mockResolvedValue(mockClient);

            // Pre-fill cache first with getUserInstruments
            (pool.query as jest.Mock).mockResolvedValueOnce({ rows: [mockInstrument] });
            await InstrumentService.getUserInstruments(userId);

            // Now call upsert
            const result = await InstrumentService.upsertInstrument(userId, {
                instrument_type: InstrumentType.CREDIT_CARD,
                bank_name: 'HDFC',
                account_number_masked: 'XX1234'
            });
            ...
            // Verify cache was invalidated by checking if next get calls DB
            jest.clearAllMocks();
            (pool.query as jest.Mock).mockResolvedValueOnce({ rows: [mockInstrument] });
            await InstrumentService.getUserInstruments(userId);
            expect(pool.query).toHaveBeenCalledTimes(1);
