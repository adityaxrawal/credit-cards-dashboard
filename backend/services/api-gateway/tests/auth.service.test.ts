...
describe('AuthService', () => {
  ...
  describe('googleOAuth', () => {
    ...
    beforeEach(() => {
      ...
      (google.auth.OAuth2 as jest.Mock) = jest.fn().mockImplementation(() => mockOAuth2);
    ...
    it('should throw error if code is invalid', async () => {
      ...
      await expect(authService.googleOAuth('invalid-code')).rejects.toThrow('Authentication failed');
    });
  ...
  describe('generateAccessToken', () => {
    ...
    it('should throw error if JWT_SECRET is missing', () => {
      ...
      expect(() => authService['generateAccessToken'](mockUser)).toThrow('JWT_SECRET');
    });
  });
  ...
  describe('generateRefreshToken', () => {
    ...
    it('should throw error if JWT_REFRESH_SECRET is missing', () => {
      ...
      expect(() => authService['generateRefreshToken'](mockUser)).toThrow('JWT_REFRESH_SECRET');
    });
  });
  ...
  describe('refreshAccessToken', () => {
    it('should generate new access token with valid refresh token', async () => {
      ...
      (redis.get as jest.Mock) = jest.fn().mockResolvedValue(JSON.stringify({
    ...
    it('should throw error if token type is not refresh', async () => {
      ...
      await expect(authService.refreshAccessToken(accessToken)).rejects.toThrow('Invalid token type');
    });
    ...
    it('should throw error if session does not exist', async () => {
      ...
      await expect(authService.refreshAccessToken(refreshToken)).rejects.toThrow('Session expired');
    });
    ...
    it('should throw error if user is inactive', async () => {
      ...
      await expect(authService.refreshAccessToken(refreshToken)).rejects.toThrow('inactive');
    });
  });
  ...
  describe('logout', () => {
    ...
    it('should handle Redis errors gracefully', async () => {
      (redis.del as jest.Mock) = jest.fn().mockRejectedValue(new Error('Redis error'));

      await expect(authService.logout(mockUser.id)).rejects.toThrow('Logout failed');
    });
  });
  ...
  describe('verifyToken', () => {
    ...
    it('should throw error for invalid token', async () => {
      await expect(authService.verifyToken('invalid-token', 'access')).rejects.toThrow();
    });

    it('should throw error for mismatched token type', async () => {
      const accessToken = authService['generateAccessToken'](mockUser);

      await expect(authService.verifyToken(accessToken, 'refresh')).rejects.toThrow('Invalid token type');
    });
    ...
    it('should throw error for expired token', async () => {
      ...
      await expect(authService.verifyToken(expiredToken, 'access')).rejects.toThrow();
    });
  });
});
