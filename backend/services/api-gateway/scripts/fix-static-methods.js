...
const alertsControllerPath = path.join(srcDir, 'modules/alerts/alerts.controller.ts');
...
alertsController = alertsController.replace(/enhancedAlertService\./g, 'EnhancedAlertService.');
...
const analyticsControllerPath = path.join(srcDir, 'modules/analytics/analytics.controller.ts');
let analyticsController = fs.readFileSync(analyticsControllerPath, 'utf8');

analyticsController = analyticsController.replace(/advancedAnalyticsService\./g, 'AdvancedAnalyticsService.');
...
const billsControllerPath = path.join(srcDir, 'modules/bills/bills.controller.ts');
let billsController = fs.readFileSync(billsControllerPath, 'utf8');

billsController = billsController.replace(/billReminderService\./g, 'BillReminderService.');
...
const budgetsControllerPath = path.join(srcDir, 'modules/budgets/budgets.controller.ts');
let budgetsController = fs.readFileSync(budgetsControllerPath, 'utf8');

budgetsController = budgetsController.replace(/enhancedBudgetService\./g, 'EnhancedBudgetService.');
...
const reportsControllerPath = path.join(srcDir, 'modules/reports/reports.controller.ts');
let reportsController = fs.readFileSync(reportsControllerPath, 'utf8');

reportsController = reportsController.replace(/reportingService\./g, 'ReportingService.');
...
billsService = billsService.replace(/enhancedAlertService\./g, 'EnhancedAlertService.');

// Ensure EnhancedAlertService is imported
if (!billsService.includes('import { EnhancedAlertService }') && billsService.includes('EnhancedAlertService.')) {
  billsService = billsService.replace(
