...
const crudSectionRegex =
  /\/\*\*\s+\* CRUD Wrapper Methods for API Controller[\s\S]*?static async deleteBill\(id: string\): Promise<void> \{[\s\S]*?\}/;
...
if (matches && matches.length > 1) {
  ...
  const secondIndex = billsService.indexOf(
    matches[1],
    firstIndex + matches[0].length
  );

  billsService =
    billsService.substring(0, secondIndex) +
    billsService.substring(secondIndex + matches[1].length);
}
...
const reportsCrudRegex =
  /\/\*\*\s+\* CRUD Wrapper Methods for API Controller[\s\S]*?static async delete\(id: string\): Promise<void> \{[\s\S]*?\}/;
...
if (reportsMatches && reportsMatches.length > 1) {
  ...
  const secondIndex = reportsService.indexOf(
    reportsMatches[1],
    firstIndex + reportsMatches[0].length
  );

  reportsService =
    reportsService.substring(0, secondIndex) +
    reportsService.substring(secondIndex + reportsMatches[1].length);
}
...
if (
  subsController.includes("supabase.from") &&
  !subsController.includes("import { supabase }")
) {
  subsController = subsController.replace(
