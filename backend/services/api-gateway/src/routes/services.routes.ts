 ...
 */
router.post("/update-budget", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { error: upsertError } = await supabase.from("budget_tracking").upsert(
      {
        user_id: userId,
        month: currentMonth,
        year: currentYear,
 ...
 * @triggeredBy Frontend after successful Gmail sync
 */
router.post("/check-alerts", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId!;
    const currentMonth = new Date().getMonth() + 1;
    const currentYear = new Date().getFullYear();
 ...
 */
router.post("/check-reminders", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId!;
 ...
 */
router.post("/refresh-analytics", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId!;
