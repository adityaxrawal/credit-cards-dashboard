...
function generateLoadTestReport() {
  ...
  console.log("📊 Load Test Report Generator");
  console.log("================================\n");
  console.log(
    "⚠️  Note: Run actual k6/Artillery tests first to generate metrics\n"
  );

  // Check for k6 results
  const k6ResultsPath = path.join(__dirname, "results", "k6-results.json");
  const artilleryResultsPath = path.join(
    __dirname,
    "results",
  ...
  if (fs.existsSync(artilleryResultsPath)) {
    const artilleryResults = JSON.parse(
      fs.readFileSync(artilleryResultsPath, "utf8")
    );
    report.results.push({
      tool: "artillery",
      ...artilleryResults,
